...
export function Contacts() {
  ...
  const docLinks = [
      { href: "#", label: t('contacts.docs.requisites') },
      { href: "#", label: t('contacts.docs.partners') },
  ];
      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-12">
            ...
            <div className="space-y-6 col-span-1 sm:col-span-2 lg:col-span-1">
                <div>
                    <p className="font-semibold text-sm text-muted-foreground mb-1">{t('contacts.managerTitle')}</p>
                    <p className="text-xl font-bold">{t('contacts.managerName')}</p>
                ...
                <div>
                    ...
                    <p className="text-base">{t('contacts.addressLine1')}<br />{t('contacts.addressLine2')}</p>
                </div>
            </div>
            ...
            {/* Documents Column */}
            <div className="col-span-1">
                <p className="font-semibold text-sm text-muted-foreground mb-4">{t('contacts.docsTitle')}</p>
                <ul className="space-y-2">
                    {docLinks.map((link) => (
                        <li key={link.label}>
                            <Link href={link.href} className="text-base hover:text-primary transition-colors">{link.label}</Link>
